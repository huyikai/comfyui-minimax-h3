...
第一步，先只描述各格里看见什么——人、衣服、站位、嘴张没张、屏幕上有没有字、点名的物件、谁相对谁。
不要用上面 OCR/转写里的词硬套，不要写「符合」「一致」。不要只写「在找」「在救」。
...
动作: 点名这一窗看见的人、物件（杆、绳、门、包、帽）、谁在谁哪一侧、谁朝谁。按看见→靠近→接触→结果的顺序写，不要收成一句情节摘要，不要编没看见的。
拿不准: 无 | 逗号分隔。只写会改读片结论的：窗内人数变了、关键的字完全看不见、关键物件看不清。口型略糊、远景路人、压缩发糊不要写。
"""
