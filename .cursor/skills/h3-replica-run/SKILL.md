...
**主对话不要通读 `frames/`。** 写稿读 ticks；只打开 `timeline.md`「必须开图」里的横条，以及拟拆段之后每段 2–3 张人最清楚的帧（写外观锁）。附录的看不清、「有对白嘴未动」不必开图——后者当**这一窗**的画外音提示，不代表整段都不入画。同一事件后一窗已经入画（远景、背影、抱柱），必须写进人数锁。说话人按台词内容判，不要按这一窗画面上是谁。ticks 的 `action` 和「它看见的」里点过名的物件、空间、动作顺序，拆段时列成动作链，对照按看见→靠近→接触→结果写满，不要从结果态起笔。
...
**故事逻辑在这一步过完。** 每写完一段，按段通读八项（说话人、因果、能力年龄、道具外观、时空、台词与动作、戏剧核、事件链）。执行错误当场改对照再回写英文；要动故事才能圆的记履历，按现稿落盘。不要等人确认草稿，也不要把逻辑审读留到生成前才做第一次。读片 ticks 点过名的物件和动作顺序必须写进对照，不要从结果态起笔。
...
8 原片事件链铺满：timeline 点过名的物件、空间、动作顺序按看见→靠近→接触→结果写全了；开场已经是结果态、过程被摘要吃掉 → 细节遗漏
...
| A 写稿 | 1–4.5 | 主 agent 先占号，子 agent 池 4 路写（落盘前过逻辑八项），主 agent 过 precheck 并复核逻辑才放行 |
...
一条链接一个子 agent。子 agent 做第 1–3 步（取材、读片、写稿落库，**含故事逻辑八项**），**不许自己判定通过**：
...
每写完一段就按段通读故事逻辑八项（说话人、因果、能力年龄、道具外观、时空、台词与动作、戏剧核、事件链）。执行错误当场改对照再回写英文；要动故事才能圆的记进段总表/发布.md，按现稿落盘。不要等人确认草稿。
写稿时把 timeline 点过名的物件和动作顺序铺进对照，不要从结果态起笔。
写完自己跑一遍 tools\precheck.py --work "人间隙/<NN-作品名>"，把 [FAIL] 改干净。
回报：落了哪几段、每段几条、precheck 最后的结果、逻辑八项改了什么/记了什么遗留、你拿不准的地方。不要出图、不要渲染。
```

它们回来之后，**主 agent 逐条自己跑 precheck、自己读 `[SPEAK]` 表、自己复核逻辑**（第 4、4.5 步）。子 agent 说过了不算过——写稿是最吃判断的一步，方差最大的也是它。
